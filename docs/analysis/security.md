...
| A-3 | Centroid topology leakage | info | accepted (near-term mitigation designed; see notes) | `flare/storage/memory.py:get_centroids` | In the current prototype, centroids are stored in plaintext in the storage service. A concrete near-term mitigation is designed but not yet implemented: store centroid maps encrypted on the grant ledger (under the oracle's master key) and deliver them to the query node inside the existing oracle key-issuance ECIES response. The query node routes locally as today — no extra round-trip, no change to routing logic, and centroid disclosure becomes gated by the same authorization boundary as cell keys. Residual mitigations if centroids are ever exposed (e.g. query node compromise): (1) Gaussian noise on the delivered map, (2) locality-preserving hashing. Documented in §7 of the paper. |
...
| O-5 | Centroid noise / locality-preserving hashing | research | Still calibration research for a post-compromise query-node scenario. The primary A-3 mitigation (encrypted centroids on the ledger, delivered inside the oracle key-issuance response) does not require noise calibration. Noise / LPH would be relevant only as a second layer if the query node itself were in the adversary's threat model. |
