...
| C-14 | Light cone propagation masks: per-edge `propagate` sets are structural (not per-principal); a `None`-mask edge excludes all traversal equally | high | `flare/lightcone.py:LightConeGraph.authorized_contexts` | `tests/test_lightcone.py::test_null_propagate_*`, `test_narrow_*` |
| C-15 | Light cone path-predicate constraints: blocks paths through a constrained node-set; alternate paths survive | high | `flare/lightcone.py:_path_constrained`, `RequireAllOf`, `RequireSequence` | `tests/test_path_predicates.py::test_require_all_of_*` |
...
| C-32 | Grant-first access: owner has no oracle fast-path; access is mediated by a standing self-grant created at bootstrap. Revoking the self-grant blocks the owner. | high | `flare/oracle/core.py:decide_batch`, `flare/bootstrap.py` | `tests/test_grant_first.py::*` |
| C-33 | Envelope encryption: CWK derived from master key via HKDF wraps random per-cell CEK via AES-256-GCM with AAD; oracle unwraps CEK on demand | high | `flare/crypto.py:derive_cwk,wrap_cek,unwrap_cek`, `flare/oracle/core.py:decide_batch` | `tests/test_envelope.py::*` |
| C-34 | Cross-context CEK re-wrapping: `share_cell_across_contexts` unwraps CEK from source CWK and wraps under target CWK; cell ciphertext is never re-encrypted | med | `flare/bootstrap.py:share_cell_across_contexts` | `tests/test_containment.py::test_cross_context_*` |
| C-35 | Containment edges: explicit graph edges express cell membership in contexts; cells can belong to multiple contexts | info | `flare/lightcone.py:LightConeGraph.containment_edges` | `tests/test_containment.py::*` |
...
| A-3 | Centroid topology leakage | info | mitigated | `flare/crypto.py:derive_centroid_key`, `flare/oracle/core.py:issue_centroids`, `flare/oracle/service.py:/request-centroids`, `flare/storage/service.py:get_centroids` (403), `tests/test_centroid_gate.py` | Centroid maps are encrypted at bootstrap under an HKDF-derived centroid key (`derive_centroid_key` with info prefix `flare/v1/centroids\x00`) and stored on the oracle as AES-256-GCM ciphertext with AAD `centroids:{context_id}`. The oracle delivers centroid maps via ECIES + Ed25519 through `POST /request-centroids` — same authorization check as cell-key issuance. Storage returns HTTP 403 for centroid requests. The query pipeline runs light-cone authorization before centroid routing, so centroids are never requested for unauthorized contexts. Residual: an authorized querier sees centroid geometry, same information it already receives via cell keys and results. Pinned by `tests/test_centroid_gate.py`. |
...
| A-12 | CWK compromise scope: compromising a context's CWK yields all CEKs in that context | med | accepted (by design) | `flare/crypto.py:derive_cwk` | The CWK is HKDF-derived from the master key, so it carries the same compromise scope as the master key for that context. The envelope adds a layer of indirection — not a smaller blast radius per context — enabling cross-context sharing and future per-grantee CWK wrapping. |
| A-13 | Super-context centroid leakage: super-context generation exposes centroid geometry to the authorized user | info | accepted (same as A-3) | `flare/supercollections.py` | Super-contexts are generated from centroids already authorized for the user. The centroid geometry is the same information the user already receives via cell-key issuance and search results. No new leakage surface. |
...
| O-5 | Centroid noise / locality-preserving hashing | research | Calibration research for a post-compromise query-node scenario. The primary A-3 mitigation is implemented (encrypted centroids delivered via oracle ECIES). Noise / LPH would be relevant only as a second layer if the query node itself were in the adversary's threat model — the authorized querier already receives centroid maps, cell keys, and decrypted results. |
