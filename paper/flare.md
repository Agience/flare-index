...
### 3.10 Envelope encryption, containment, and super-contexts

Three extensions stabilize the architecture for composition across contexts and users.

**Grant-first access.** The oracle has no `requester == owner` fast-path. Owner access is mediated by a standing self-grant created at bootstrap and stored in the ledger. Revoking the self-grant blocks the owner — the ledger is the sole authority. This composes with delegation and multi-tenant access: every principal, including the data owner, follows the same authorization path through the grant ledger. See `tests/test_grant_first.py`.

**Envelope encryption (CWK + CEK).** The single-layer HKDF derivation (`master_key → cell_key`) is replaced with a two-layer envelope:

- **CWK (Context Wrapping Key)**: per-context, HKDF-derived from the master key. The oracle derives it on the fly, same as before.
- **CEK (Cell Encryption Key)**: per-cell, random. Wraps the cell data via AES-256-GCM. The CEK is wrapped by the CWK (also via AES-256-GCM with AAD binding) and stored alongside the ciphertext.

At key-issuance time the oracle derives the CWK from the master key, unwraps the CEK, and returns it to the querier. Cell data is never re-encrypted. Cross-context sharing is implemented by re-wrapping the same CEK under a different context's CWK (`share_cell_across_contexts` in `flare/bootstrap.py`). See `tests/test_envelope.py`.

**Containment edges.** Explicit edges in the light-cone graph express which cells belong to which contexts. A cell can belong to multiple contexts without document-level conflict — containment is a graph relationship, not a field on the cell. The query engine resolves cells from containment edges when available. See `tests/test_containment.py`.

**Super-contexts (KNN-generated).** Per-user auto-clustered groupings from the union of all centroids visible in the user's light cone. Generated by k-means over concatenated centroid vectors, scoped per-user (different light cones produce different clusters), ephemeral (not stored), and composable with the existing query pipeline as optional filters. Implemented in `flare/supercollections.py`; tested in `tests/test_super_contexts.py`.
...
- **CWK compromise scope is per-context.** The envelope encryption (§3.10) decouples cell encryption keys from master-key derivation but does not reduce the blast radius of a CWK compromise — an attacker who obtains a context's CWK can unwrap every CEK in that context. The CWK is HKDF-derived from the master key, so its compromise scope is identical to the master key's scope for that context. Future work: per-grantee CWK wrapping, where each grantee receives the CWK wrapped under their public key, enabling revocation by removing wrapped copies.
- **Super-contexts are ephemeral and unverified.** Super-context generation (§3.10) clusters centroids that the user is already authorized to see, so no new leakage surface exists. However, the clustering result is not signed or verifiable — a compromised query node could present fabricated clusters. In the sidecar deployment model this is moot (the query node is the querier's own process), but shared infrastructure deployments would need TEE-hosted clustering.
